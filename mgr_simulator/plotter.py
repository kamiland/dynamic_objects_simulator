...
t = 0
try:
    with open('./data/state.txt', 'r') as csvfile:
        ...
        plt.figure()
        plt.plot(T, x0, label="Pozycja wózka - \u03B80 [m]", color="blue")
        plt.plot(T, x1, label="Pozycja ramienia 1 - \u03B81 [rad]", color="orange")
        plt.plot(T, x2, label="Pozycja ramienia 2 - \u03B81 [rad]", color="green")
        # plt.plot(T, x3, label="Prędkość wózka - \u03B80' [m/s]", linestyle=':', color="blue")
        # plt.plot(T, x4, label="Prędkość ramienia 1 - \u03B81' [rad/s]", linestyle=':', color="orange")
        # plt.plot(T, x5, label="Prędkość ramienia 2 - \u03B82' [rad/s]", linestyle=':', color="green")
        # plt.plot(T, x6, label="Wyjście z sieci neuronowej - U [N]", color="red")
        plt.plot(T, setp, label="\u03C0; -\u03C0", linestyle='--', color="grey")
        plt.plot(T, setp2, linestyle='--', color="grey")
        plt.grid(linestyle=':')
        leg = plt.legend()
