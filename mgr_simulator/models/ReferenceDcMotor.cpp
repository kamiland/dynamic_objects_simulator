#include "ReferenceDcMotor.hpp"

ReferenceDcMotor::ReferenceDcMotor(double rotorCurrent, double angularVelocity)
{
    /** Parametry domyślne  */
    Ra = 0.4;   
    La = 0.02;
...
ReferenceDcMotor::~ReferenceDcMotor()
{
}


double ReferenceDcMotor::calculateRotorCurrent(double X1, double X2, double U)
{
    return -(Ra / La) * X1 - (Gaf / La) * X2 + (1 / La) * U;
}


double ReferenceDcMotor::calculateAngularVelocity(double X1, double X2, double Tl)
{
...
double * ReferenceDcMotor::CalculateNextStep(double U, double h)
{
...
vector<DcMotorState> ReferenceDcMotor::Simulate(long numberOfProbes, double timeStep)
{
    X[0] = 0; // rotor current
